...
import {
  ...
  getVerificationTokenByWhoRequested,
...
export const settings = async (values) => {
  ...
  if (values.email && values.email !== user.email) {
    ...
    if (existingVerificationToken) {
      ...
      if (!hasExpired) {
        return {
          error: "Verification email already sent! Confirm your inbox!",
        };
      }
    }

    /* Grace period before user can request new email change */
    const verificationTokenByRequest_email_change_by =
      await getVerificationTokenByWhoRequested(dbUser.email);
    if (verificationTokenByRequest_email_change_by) {
      const hasExpired =
        new Date(verificationTokenByRequest_email_change_by.expires) <
        new Date();
      if (!hasExpired) {
        return {
          error:
  ...
  const updatedUser = await connectToDb.user.update({
    where: { id: dbUser.id },
    data: { ...values },
  });
